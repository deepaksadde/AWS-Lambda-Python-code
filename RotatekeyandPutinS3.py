...
def upload_into_s3(emailid,access_key,secret_key):
    
   #f = open('/tmp/aws-credential-file.csv', "w+")
   bucket = 'truehelper'
   file = emailid + '-aws_credentials.csv'
   with open('/tmp/aws-credential-file.csv', 'w+', newline='') as f:
    wr = csv.writer(f) 
    fields = ['Email', 'AWSAccessKey', 'AWSSecretAccessKey']
    wr.writerow(fields)
    wr.writerow([emailid, access_key, secret_key])
   f.close()

   # upload to s3
   client = boto3.client('s3')
   client.upload_file('/tmp/aws-credential-file.csv', bucket,file)
...
def lambda_handler(event, context):
    ...
    for _ in user_iam_details:
        ...
        upload_into_s3(emailid,access_key,secret_key)
