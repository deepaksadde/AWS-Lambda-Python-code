...
def send_access_secret_key(emailid,access_key,secret_key):
    
    CHARSET = "UTF-8"
    BODY =  "Access key {access_key}  Secret key {secret_key} "
    try:         
     response = ses_client.send_email(
        ...
        Message={
            ...
            "Subject": {
                "Charset": CHARSET,
                "Data": "Access and Secret Key has been rotated",
            },
        },
        Source=emailid,
     )
...
def lambda_handler(event, context):
    ...
    user='terraform_user'
    user_iam_details=list_access_key(user=user,days_filter=40,status_filter='Active')
    for _ in user_iam_details:
        ...
        send_access_secret_key(emailid,access_key,secret_key)
